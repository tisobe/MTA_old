...
#########################################################################################
#											#
#	alignment_sim_twist_extract_plot_part.perl: make a plot after extracted data    #
#											#
...
sub plot_data{
	if($lyear =~ /\d/){
		$data   = 'data_extracted_'."$lyear";
		$in_list = `ls $web_dir/Data/$data`;
	}else{
...
print "I AM HERE5\n";
	...
	pgbox(ABCNSTV,0.0 , 0.0, ABCNSTV, 0.0, 0.0);
	pgptxt($xside,$ymid_dtheta, 90.0, 0.5, "dtheta (arc sec)");
	@ybin = @dtheta;
	plot_fig();
	pgsci(3);
...
#		$ymax = $avg + 0.01;
		...
		pgbox(ABCNST,0.0 , 0.0, ABCNSTV, 0.0, 0.0);
        	pgptxt($xside2,$ymid, 90.0, 0.5, "dtheta (arc sec)");
