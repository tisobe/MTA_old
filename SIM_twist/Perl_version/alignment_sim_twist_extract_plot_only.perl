...
###	system("echo ''|$op_dir/gs -sDEVICE=ppmraw  -r256x256 -q -NOPAUSE -sOutputFile=-  ./Sim_twist_temp/pgplot.ps| $op_dir/pnmflip -r270 |$op_dir/ppmtogif > $web_dir/Plots/sim_plot.gif");
...
$ymax_dtheta =  80;
	...
	$ymax = $dy_int3 + $dy_slope3 * 4400;
	pgmove(2700, $ymin);
	pgdraw(4400, $ymax);
	$ymin = $dy_int4 + $dy_slope4 * 4400;
	$ymax = $dy_int4 + $dy_slope4 * $xmax;
	pgmove(4400, $ymin);
	pgdraw($xmax, $ymax);
	pgsci(1);
	...
	$ytop4 = $ymax_dy - 0.40 * $diff; 
	pgptxt($xdpos,$ytop1, 0.0, 0.0, "Slope (dom < 1400): $dy_disp1");
	pgptxt($xdpos,$ytop2, 0.0, 0.0, "Slope (1400 < dom < 2700): $dy_disp2");
	pgptxt($xdpos,$ytop3, 0.0, 0.0, "Slope (2700 < dom > 4400): $dy_disp3");
	pgptxt($xdpos,$ytop4, 0.0, 0.0, "Slope (dom > 4400): $dy_disp4");
	...
	$ymax = $dz_int3 + $dz_slope3 * 4400;
	pgmove(2700, $ymin);
	pgdraw(4400, $ymax);
	$ymin = $dz_int4 + $dz_slope4 * 4400;
	$ymax = $dz_int4 + $dz_slope4 * $xmax;
	pgmove(4400, $ymin);
	pgdraw($xmax, $ymax);
	pgsci(1);
	$xdpos = $xmin + 0.04 * $xdiff;
	...
	$ytop4 = $ymax_dz - 0.40 * $diff; 
	pgptxt($xdpos,$ytop1, 0.0, 0.0, "Slope (dom < 1400): $dz_disp1");
	pgptxt($xdpos,$ytop2, 0.0, 0.0, "Slope (1400 < dom < 2700): $dz_disp2");
	pgptxt($xdpos,$ytop3, 0.0, 0.0, "Slope (2700 < dom < 4400): $dz_disp3");
	pgptxt($xdpos,$ytop4, 0.0, 0.0, "Slope (dom > 4400): $dz_disp4");
...
###	system("echo ''|$op_dir/gs -sDEVICE=ppmraw  -r256x256 -q -NOPAUSE -sOutputFile=-  ./Sim_twist_temp/pgplot.ps| $op_dir/pnmflip -r270 |$op_dir/ppmtogif > $web_dir/Plots/twist_plot.gif");
...
###    	system("echo ''|$op_dir/gs -sDEVICE=ppmraw  -r256x256 -q -NOPAUSE -sOutputFile=-  ./Sim_twist_temp/pgplot.ps| $op_dir/pnmflip -r270 |$op_dir/ppmtogif > $web_dir/Plots/dtheta_plot.gif");
