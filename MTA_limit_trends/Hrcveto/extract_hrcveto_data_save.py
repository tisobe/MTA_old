...
def find_the_last_entry_time():
    """
    find the last entry date and then make a list of dates up to yesterday
    input:  none
    output: otime       --- a list of dates in the format of yyyymmdd
    """
#
...
#
    ltime = ltime + 86400.0
#
#--- find yesterday's time
#
    ct    = time.strftime('%Y%m%d', time.gmtime())
    year  = int(ct[0:4])
    mon   = int(ct[4:6])
    day   = int(ct[6:8])
    dt    = datetime.datetime(year, mon, day)
    stime = time.mktime(dt.timetuple()) - 86400.0
