...
#################################################################################
#                                                                               #
#       acis_sci_run_err3x3.perl: extract Te5x5 data and if there is high       #
...
@old_list = ();
open(FH, "$root_dir/$current_dir/high_error5x5_$uyear");
while(<FH>){
        chomp $_;
        @atemp = split(//, $_);
        if($atemp[0] =~ /\d/){
...
open(OUT, ">$root_dir/$current_dir/high_error5x5_$uyear");	#------ print out record
...
#
		if($atemp[2] =~ /ACIS/ && $atemp[4] =~ /Te5x5/ && $ratio > 1.0){
			@etemp = split(//, $ratio);
...
for($year = 1999; $year <= $uyear; $year++){
        $f_name = "$root_dir".'/Year'."$year".'/high_error5x5_'."$year";
        open(FH, "$f_name");
        print OUT "\nYear $year\n";
	print OUT "----------\n";
        while(<FH>){
...
close(OUT);
system("mv ./Working_dir/temp_warning $root_dir/Long_term/high_error5x5");
...
if($chk > 0){
	...
	print OUT "ACIS Science Run issued the following warning(s)\n\n";
        print OUT "The following observation has a high Te5x5 Error Rate in today's science run\n\n";
	print OUT "obsid   target                  start time  int time        inst    ccd     grat    err/ksec    \n";
        print OUT "---------------------------------------------------------------------------------------------------\n";
        foreach $ent (@alart){
                print OUT "$ent\n";
