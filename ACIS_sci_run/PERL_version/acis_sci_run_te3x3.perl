...
#################################################################################
#										#
#	acis_sci_run_te3x3.perl: extract Te3x3 data and if there is drop rate	#
...
open(FH, "$root_dir/$current_dir/drop_$uyear");
@old_list = ();
while(<FH>){
	chomp $_;
	@atemp = split(//, $_);
...
open(OUT, ">$root_dir/$current_dir/drop_$uyear");	#------ print out record
...
#
	if($atemp[2] =~ /ACIS/ && $atemp[4] =~ /Te3x3/ && $atemp[9] > 3.0){
                $cnt = 0;
...
for($year = 1999; $year <= $uyear; $year++){
        $f_name = "$root_dir".'/Year'."$year".'/drop_'."$year";
        open(FH, "$f_name");
        print OUT "\nYear $year\n";
	print OUT "----------\n";
        while(<FH>){
...
close(OUT);
system("mv ./Working_dir/temp_warning $root_dir/Long_term/drop");
...
if($chk > 0){
	...
	print OUT "The following observation has a high Te3x3 Drop Rate in today's science run\n\n";
	print OUT "obsid   target                  start time      int time        inst    ccd     grat    drop rate(%)\n";
	print OUT "-------------------------------------------------------------------------------------------------------\n";
	foreach $ent (@alart){
		print OUT "$ent\n";
