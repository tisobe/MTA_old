...
#########################################################################################
#	acis_sci_run_high_evnt3x3.perl: extract Te3x3 data and if there is high 	#	
#					event observation (cnt/sec > 180), take a note	#
...
open(FH, "$root_dir/$current_dir/high_event_$uyear");
@old_list = ();
while(<FH>){
        chomp $_;
        @atemp = split(//, $_);
...
@new_list = ();
open(FH, "$input");
open(OUT, ">$root_dir/$current_dir/high_event_$uyear");
...
#
		if($atemp[2] =~ /ACIS/ && $atemp[4] =~ /Te3x3/ && $ratio > 180.0){
			@etemp = split(//, $ratio);
...
for($year = 1999; $year <= $uyear; $year++){
        $f_name = "$root_dir".'/Year'."$year".'/high_event_'."$year";
        open(FH, "$f_name");
        print OUT "\nYear $year\n";
	print OUT "----------\n";
        while(<FH>){
...
close(OUT);
system("mv ./Working_dir/temp_warning $root_dir/Long_term/high_event");
...
if($chk > 0){
	...
	print OUT "ACIS Science Run issued the following warning(s)\n\n";
        print OUT "The following observation has a high Te3x3 Event Rate in today's science run\n\n";
        print OUT "obsid   target                  start time	int time        inst    ccd     grat    event rate (sec)\n";
        print OUT "---------------------------------------------------------------------------------------------------------\n";
        foreach $ent (@alart){
                print OUT "$ent\n";
        }
