...
#########################################################################################
#											#
#       acis_sci_run_high_evnt5x5.perl: extract Te5x5 data and if there is high         #
#                                       event observation (cnt/sec > 56), take a note   #
#                                       and send out email notice                       #
#                                                                                       #
...
open(FH, "$root_dir/$current_dir/high_event5x5_$uyear");
@old_list = ();
while(<FH>){
        chomp $_;
        @atemp = split(//, $_);
...
@new_list = ();
open(FH, "$input");
open(OUT, ">$root_dir/$current_dir/high_event5x5_$uyear");
print OUT  "obsid   target                  start time 	int time        inst    ccd     grat    avg # of events/sec\n";
print OUT  "-----------------------------------------------------------------------------------------------------------\n";
while(<FH>){
	chomp $_;
	@atemp = split(/\t/, $_);
	if($atemp[6] =~ /\d/ && $atemp[6] > 0){
		$ratio = $atemp[7]/$atemp[6]/1000;
#
#------ here is the selection criteria
#
		if($atemp[2] =~ /ACIS/ && $atemp[4] =~ /Te5x5/ && $ratio > 56.0){
			@etemp = split(//, $ratio);
...
for($year = 1999; $year <= $uyear; $year++){
        $f_name = "$root_dir".'/Year'."$year".'/high_event5x5_'."$year";
        open(FH, "$f_name");
        print OUT "\nYear $year\n";
	print OUT "----------\n";
        while(<FH>){
...
close(OUT);
system("mv ./Working_dir/temp_warning $root_dir/Long_term/high_event5x5");
...
if($chk > 0){
	...
	print OUT "ACIS Science Run issued the following warning(s)\n\n";
        print OUT "The following observation has a high Te5x5 Event Rate in today's science run\n\n";
	print OUT "obsid   target                  start time  int time        inst    ccd     grat    avg # of events/sec\n";
        print OUT "-------------------------------------------------------------------------------------------------------\n";
        foreach $ent (@alart){
                print OUT "$ent\n";
