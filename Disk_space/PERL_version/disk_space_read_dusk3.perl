...
#--- read the past data
#
open(FH, "$data_out/disk0_data_mays");
while(<FH>){
	chomp $_;
	@atemp = split(/:/, $_);
	push(@time, $atemp[0]);
...
#-- update the database
#
open(OUT, ">$data_out/disk0_data_mays");
for($i = 0; $i < $dcnt; $i++){
	print OUT "$time[$i]";
	for($k = 0; $k < $name_num; $k++){
		print OUT ":${data.$k}[$i]";
...
system("echo ''|/opt/local/bin/gs -sDEVICE=ppmraw  -r256x256 -q -NOPAUSE -sOutputFile=-  pgplot.ps| $bin_dir/pnmflip -r270 | $bin_dir/pnmcrop -bottom | $bin_dir/ppmtogif > $fig_out/data_mays.gif");
system('rm pgplot.ps');
