...
dir = '/data/mta/Script/Ephem/EPH_Data/'
...
;
;---- following line is modified 12/04/14 by t.i.
;
;nbytes=long(info(1))       
nbytes=long(info(0))
nrec = nbytes/2800
    ...
    for j=0,49 do begin
      if (px(j) lt 1d22 and valid_data gt 0) then begin
        ...
        printf,olun, sarr(j),px(j),py(j),pz(j),vx(j),vy(j),vz(j),$
                     fyarr(j),month(j),day(j),hour(j), $
                     minute(j),second(j), $
        format='(7d16.3,f12.6,5(i4))' 
      endif else begin
         valid_data = 0
      endelse
   endfor
